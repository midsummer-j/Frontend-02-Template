// 1，position display float 古典
// 2，flex 
// 3，grid
// 在CSS的基础上，通过排版计算，得到一个带位置的DOM树
/**分行
 * 根据主轴尺寸，把元素分进行
...
function layout(element) {
    ...
    if (!style.flexWrap || style.flexWrap === "auto") style.flexWrap = 'flex-nowrap';
    ...
    flexLines.forEach(function (items) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if(align === 'stretch'){
                itemStyle[crossStart] = crossBase;
                itemStyle[crossEnd] = crossBase + crossSign * ((itemStyle[crossSize] !== null && itemStyle[crossSize] !==(void 0)) ? 0 : lineCrossSize);

                itemStyle[crossSize] = itemStyle[crossSize]|| crossSign * (itemStyle[crossEnd] - itemStyle[crossStart])
            }
        }
